...
Q_MAX = 11          # roadmap standing rule: never run q >= 13; default cap q <= 11
ENCODINGS = ("per_series",)
...
def derive(n_series: int, encoding: str = "per_series", mem_rule: str = "fixed",
           ...
           n_taus: int = 1, qrc_window: int = 1, poly2_window: int = 1, q_max: int = Q_MAX) -> Layout:
    ...
    if mem_rule == "fixed":
        ...
        n_mem = next((m for m in range(0, q_max - n_in + 1) if width(m) >= p2), None)
        if n_mem is None:
