...
def generate_drift(n_steps, seed, period, noise=0.02, burn=200,
                    phi_base=0.0, drift_amp=0.85):
    """C6b second nonstationarity: the AR coefficient drifts CONTINUOUSLY
    (sinusoidally) instead of switching discretely.  Same 'model must track a
    moving regime' challenge as generate_regime_switching, different structure
    (smooth drift vs jumps).

      phi_t = phi_base + drift_amp * sin(2*pi*t / period)
      x_{t+1} = phi_t * x_t + noise

    `period` sets how fast the regime moves (advantage axis: smaller period =
    faster drift = harder for a fixed-window model).  Returns x (standardized)
    and the continuous regime signal phi_t (the 'hidden state' analogue)."""
    rng = np.random.default_rng(seed)
    x = rng.uniform(-0.5, 0.5)
    xs, phis = [], []
    for t in range(n_steps + burn):
        ...
        x = min(max(x, -5.0), 5.0)
        xs.append(x); phis.append(phi)
    x = np.array(xs[burn:]); phi = np.array(phis[burn:])
    xstd = (x - x.mean()) / (x.std() + 1e-12)
    return xstd, phi
...
def self_test():
    ...
    # G1'': C6b continuous-drift task also requires regime (phi_t) inference --
    # a phi-oracle beats a fixed-window linear model, gap grows as drift speeds up
    # (smaller period).  Same logic as G1, adapted to smooth drift.
    print("[G1''] continuous-drift data: bounded + task requires drift tracking "
          "(oracle gap grows as period shrinks)")
    dgaps = []
    for period in (100000, 400, 150):    # near-stationary -> fast drift
        x, phi = generate_drift(4000, seed=0, period=period)
        assert np.all(np.isfinite(x)), f"non-finite at period={period}"
        Wlin = windowed(x[:, None], 3)
        r2_fixed = _r2(Wlin[:-1], x[1:, None])
        # phi-oracle: window PLUS the hidden phi-coefficient interaction phi_t*x_t
        Worc = np.concatenate([Wlin, (phi[:, None] * x[:, None])], axis=1)
        r2_oracle = _r2(Worc[:-1], x[1:, None])
        gap = r2_oracle - r2_fixed
        dgaps.append(gap)
        print(f"   period={period}: bounded[{x.min():.2f},{x.max():.2f}] "
              f"fixed_R2={r2_fixed:.3f} oracle_R2={r2_oracle:.3f} gap={gap:.3f}")
    g1d = np.all(np.isfinite(x)) and dgaps[0] < 0.05 and dgaps[2] > dgaps[0] + 0.1
    print(f"   oracle-over-fixed gap grows as drift speeds up: "
          f"{'PASS' if g1d else 'FAIL'}")
    ok &= g1d
...
def phase2(n1=1, n2=4, n_steps=1200, p_switch=0.05, horizon=1, window=5,
            ...
            dataset="switching", drift_period=400):
    ...
    dataset='switching' (Case II) or 'drift' (C6b continuous-drift task)."""
    q = n1 + n2; assert q <= 11
    ...
    for ds in data_seeds:
        if dataset == "switching":
            ...
            x, s = generate_drift(n_steps, seed=ds, period=drift_period)
        else:
    ...
    out = {"config": {"n1": n1, "n2": n2, "q": q, "n_steps": n_steps,
                       ...
                       "dataset": dataset, "drift_period": drift_period},
            "nrmse": {m: acc[m] for m in methods},
            "mda": {m: accM[m] for m in methods},
            "persistence_mda": persistM}
    tag = "" if dataset == "switching" else f"_{dataset}"
    fp = outdir / f"phase2{tag}_ps{p_switch}_h{horizon}.json"
    fp.write_text(json.dumps(out, indent=2))
    print(f"  Saved {fp}  (sha {hashlib.sha256(fp.read_bytes()).hexdigest()[:12]})")
    return out
...
def main():
    ...
    ap.add_argument("--n-seeds", type=int, default=3,
                     help="use S data seeds x S reservoir seeds in phase2")
    ap.add_argument("--dataset", choices=["switching", "drift"],
                     default="switching")
    ap.add_argument("--drift-period", type=int, default=400)
    args = ap.parse_args()
    if args.self_test:
        self_test(); return
    if args.phase1:
    ...
    if args.phase2:
        ...
        print(f"=== Phase 2: FB-QRC vs tuned ESN/Poly2/open-loop "
              f"(dataset={args.dataset}, p_switch={args.p_switch}, "
              f"horizon={args.horizon}, {S}x{S} seeds) ===")
        phase2(p_switch=args.p_switch, horizon=args.horizon,
                data_seeds=seeds, res_seeds=seeds,
                dataset=args.dataset, drift_period=args.drift_period); return
