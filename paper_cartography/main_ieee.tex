...
simulation harness. The scope is stated plainly: at sizes classical hardware
represents exactly, the claim is ``no advantage here, under fair comparison,''
not ``no advantage is possible.''
...
validation error. The classical baselines receive the \emph{same} tuning budget
as the quantum model: the single most important control and the one most often
missing.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_caseI.pdf}
...
input window. Path-dependence is exactly what a classical recurrent network also
provides, so the honest baseline is the echo-state network (ESN), and the test is
whether \emph{quantum} recurrence beats \emph{classical} recurrence at equal
tuning.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_caseII.pdf}
