...
we find the advantage disappears once the comparison is fair. Extra quantum
...
conditions are often not met: baselines run at default settings while the quantum
model is tuned, ``classical reservoir'' names a different architecture, efficiency
is measured as simulation wall-clock time, and directional accuracy is reported
without a persistence control.
...
broader fair-comparison cartography (high-body trajectory observables,
channel-spectrum learning, cross-asset volatility spillover) are deferred to a
companion treatment, to which the same methodology applies.
...
self-test verifies that the reservoir state is physical ($\mathrm{Tr}\,\rho=1$,
expectations in $[-1,1]$), that the two-point readout from the density-matrix
diagonal matches the explicit operator expectation to machine precision, that
finite-shot estimates converge to the exact values, and that the feedback path is
strictly causal and reproduces the open-loop reservoir at gain zero. Every
reported number is read back from disk against a content hash; all results are
deterministic and reproduce bit-for-bit.
...
The result files and analysis scripts that reproduce every table and figure are
at \url{https://github.com/pandey-tushar/Quantum_Chaos_solver} (branch
\texttt{paper-7-cartography}), including the per-seed JSONs for Case~I and~II, the
experiment and figure scripts, and a README mapping each table to its command. All
results reproduce bit-for-bit from the provided random seeds.
